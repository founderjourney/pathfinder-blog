---
title: "Cómo Elegí la Arquitectura Multi-Tenant para HostelOS"
excerpt: "Tenía dos clientes esperando para usar HostelOS en producción. El sistema estaba listo, pero diseñado para un solo hostal. Documenté mi proceso de evaluación de 3 estrategias multi-tenant y por qué la decisión obvia estaba equivocada."
date: "2025-12-26"
...
# Cómo Elegí la Arquitectura Multi-Tenant para HostelOS
...
HostelOS es un Property Management System que construí para resolver la fragmentación de canales en hostales pequeños.
...
El sistema funcionaba perfectamente. Pero cada tabla —`guests`, `beds`, `bookings`, `transactions`— asumía un único hostal. No existía el concepto de "este huésped pertenece a este cliente".
...
- Posts de ingenieros de empresas como Supabase, Notion y Linear
...
Con Database per Tenant (mi propuesta):
...
Dividí el trabajo en 2 sprints:

**Sprint 1: Schema & Data Foundation**
- Crear tabla tenants
- Agregar tenant_id a las 30+ tablas
- Crear índices + habilitar RLS
- Migrar datos existentes a tenant_id=1 (Training)

**Sprint 2: Backend Integration**
- Middleware de tenant resolution
- Modificar login para incluir tenant en sesión
- Adaptar db-adapter para setear RLS automáticamente
- Testing E2E de aislamiento
- Deploy zero-downtime + scripts de administración

---

## Comparación Final

| Aspecto | DB per Tenant | Shared DB + RLS |
|---------|---------------|-----------------|
...
| Seguridad de datos | Aislamiento físico | RLS (igualmente seguro) |
...
*¿Preguntas o experiencias similares? Conectemos en [LinkedIn](https://linkedin.com/in/founder-journey) o sígueme en [founderjourney.com](https://founderjourney.com) donde documento el proceso de construir SaaS en público.*
